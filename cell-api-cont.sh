...
export CONTROLLER_HOST=172.16.0.101
...
CINDER_ENDPOINT=$(ifconfig em1 | awk '/inet addr/ {split ($2,A,":"); print A[2]}' | sed 's/\.[0-9]*$/.211/')
...
sudo apt-get -y install rabbitmq-server nova-novncproxy novnc nova-api nova-ajax-console-proxy nova-cert nova-consoleauth nova-doc python-novaclient dnsmasq nova-objectstore nova-cells
